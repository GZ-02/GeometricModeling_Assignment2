...
public class Registration extends PjWorkshop {
	...
	/**** Function to discard points from P according to median distance ****/
	public void DiscardPointsFromP(int k, PdVector pvec){
		double median=0.0;
		for (int j=0; j<pvec.getSize();j++){
			median=median+pvec.getEntry(j);
		}
		median=median/pvec.getSize();
		for (int i=0;i<pvec.getSize();i++){
			// Remove points from distance vector and index list
			if (pvec.getEntry(i) > k*median){
				//pvec.setEntry(k,-100.0);
				PointsOfSurfaceP.remove(i);
				pvec.removeEntry(i);
			}
		}
	}
